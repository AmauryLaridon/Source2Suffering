...
% autoscaling off
zgrid = mf_gridfit(xs,ys,z,xis,yis,'autoscale','off');

% autoscaling on (the default)
zgrids = mf_gridfit(xs,ys,z,xis,yis,'autoscale','on');
...
z = exp(x+y);
nodes = -3:.1:3;
zg = mf_gridfit(x,y,z,nodes,nodes,'reg','gradient','sm',.01);
zl = mf_gridfit(x,y,z,nodes,nodes,'reg','laplacian','sm',.01);
zs = mf_gridfit(x,y,z,nodes,nodes,'reg','springs','sm',.01);
...
[zg,xg,yg] = mf_gridfit(x,y,z,xnodes,ynodes,'tilesize',120,'overlap',0.25);
