function [zgrid,xgrid,ygrid] = mf_gridfit(x,y,z,xnodes,ynodes,varargin)
...
%          amount of smoothing.
%
% usage #1: zgrid = mf_gridfit(x,y,z,xnodes,ynodes);
% usage #2: [zgrid,xgrid,ygrid] = mf_gridfit(x,y,z,xnodes,ynodes);
% usage #3: zgrid = mf_gridfit(x,y,z,xnodes,ynodes,prop,val,prop,val,...);
%
% Arguments: (input)
...
%  ynodes = 0:.1:1;
%
%  g = mf_gridfit(x,y,z,xnodes,ynodes);
%
...
while ~isempty(xtind) && (xtind(1)<=nx)
  ...
  while ~isempty(ytind) && (ytind(1)<=ny)
    ...
    if length(k)<4
      ...
      % build this tile
      zgtile = mf_gridfit(x(k),y(k),z(k),xnodes(xtind),ynodes(ytind),Tparams);
