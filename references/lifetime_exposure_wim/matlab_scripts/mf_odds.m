function [output] = mf_odds(varargin)
%ODDS
...
% create output vairable - WT
output.alpha        = alpha;      % significance level [] 
output.rr           = rr;         % risk ratio []
output.rr_ci        = rrci;       % risk ratio confidence interval []
output.abs_risk_red = d   .* 100; % [%]
output.rel_risk_red = rrr .* 100; % [%]
output.or           = or;         % risk ratio []
output.or_ci        = orci;       % odds ratio confidence interval []
