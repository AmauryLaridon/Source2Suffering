...
function [var_ap, varargout] = mf_fieldmax(var, varargin)
...
for i=1:length(varargin)
    ...
    if    length(size(var)) == 2
        ...
        % get field mean
        var_ap = nanmax(nanmax(var));
        var_mp = nanmax(nanmax(var(mask)));

    elseif length(size(var)) == 3

        % get field mean for all pixel
        var_ap = nanmax(nanmax(var,1),2);
        ...
        for j = 1:length(var(1,1,:))
            vari        = var(:,:,j);
            var_mp(j,1) = nanmax(nanmax(vari(mask),1),2);
        end
