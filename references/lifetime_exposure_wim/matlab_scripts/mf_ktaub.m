...
function [taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = mf_ktaub(datain, alpha, wantplot)
    ...
    if sig<=alpha && wantplot ~= 0  %A plotting example CLD added 5/3/2006
        ...
        plot(datain(:,1),datain(:,2), 'ro')
        plot(datain(:,1),slope,'r-', 'linewidth',1.5)
        ...
        legend(['Gisenyi (' num2str(length(datain(:,1))) ' years)'],...
               ['Teil-Sen trend estimate: ' num2str(round(sen*1000)/1000) '°C yr^-^1']                         ,...
               [num2str((1-alpha)*100) '% confidence slopes']                     , 2);
        set(legend,'YColor','w','XColor','w', 'Fontweight', 'Bold', 'Fontsize', 10);
        set(gca, 'Fontsize', 10, 'Fontweight', 'Bold');
        xlabel('year', 'Fontweight', 'Bold', 'Fontsize', 14);
        ylabel('annual mean T [°C]', 'Fontweight', 'Bold', 'Fontsize', 14);
        box on
        grid on
        hold off
        %         pause
