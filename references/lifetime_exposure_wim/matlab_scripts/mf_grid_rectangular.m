function grid_rectangular (xstart, xend, xmin, xmax, xnum, ystart, yend, ymin, ymax, ynum )
...
%
  if ( xnum == 1 )
    x = ( xmin + xmax ) / 2;
    line ( [ x, x ], [ ystart, yend ], 'LineStyle', ':', 'Color', color );    
  else
    x = linspace ( xmin, xmax, xnum );
    for i = 1 : xnum
      line ( [ x(i), x(i) ], [ ystart, yend ], 'LineStyle', ':', 'Color', color );
    end
  end
%
%  Draw horizontals.
%
  ...
  if ( ynum == 1 )
    y = ( ymin + ymax ) / 2;
    line ( [ xstart, xend ], [ y, y ], 'LineStyle',':', 'Color', color );    
  else
    y = linspace ( ymin, ymax, ynum );
    for i = 1 : ynum
      line ( [ xstart, xend ], [ y(i), y(i) ], 'LineStyle', ':', 'Color', color );
    end
  end
