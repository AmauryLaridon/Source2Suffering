...
function [var_ap, varargout] = mf_fieldpctile(var, percentile, varargin)
...
for i=1:length(varargin)
    ...
    if    length(size(var)) == 2
        ...
        % get field median
        var_ap = prctile(var(:)   , percentile);
        var_mp = prctile(var(mask), percentile);       
    ...
    elseif length(size(var)) == 3
        ...
        for j = 1:nz
            vari        = var(:,:,j);
            var_ap(j,1) = prctile(vari(:)   , percentile);
            var_mp(j,1) = prctile(vari(mask), percentile);
        end
