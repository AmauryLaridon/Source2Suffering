...
function [out, out_pol_lat, out_pol_lon] = mf_srex(lat, lon, var, area, island, user_mask, varargin)
...
% [T2M_EAF, T2M_SAF] = mf_srex(lat_mod, lon_mod, T_2M, area, island, [], 'EAF', 'SAF');
...
for i=1:length(varargin)
    ...
    % % debugging: plot mask
    % % mf_plot_dom2(lon, lat, double(mask), [], island, [0 1], mf_colormap_cpt('BrBG_03', 2), 0, 2, ' ', label, ' ');
 

    % get mean over the srex region (land pixels only)
    [~, var_srex] = mf_fieldmean(var, area, mask);
