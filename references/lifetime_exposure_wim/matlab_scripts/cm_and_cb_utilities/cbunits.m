...
%           cbunits ºC nospace     % Do not add a space before the unit.
%           cbunits ºC off
%           cbunits(H,'°C',...)
%     CBH = cbunits(...);
%
...
%   ---
%   MFILE:   cbunits.m
%   VERSION: 4.0 (Jun 05, 2014) (<a href="matlab:web('http://www.mathworks.com/matlabcentral/fileexchange/authors/11258')">download</a>)
...
arrayrep = @(x,ind,val) ... x(ind) = val
    subsasgn(x,struct('type','()','subs',{{ind}}),val)+0;
oom = @(x) ... % Order of magnitud of x
    arrayrep(floor(log10(abs(x))),x==0,0) + ...
    arrayrep(zeros(size(x)),~isfinite(x),NaN);
sig = @(x) ... % Significant digits on x
    bsxfun(@(y,sigmay) ...
        min(...
            arrayrep(sigmay,sigmay==0,Inf), ...
            cell2mat( ...
                regexp( ...
                    arrayfun(@(sigmaz,z) ...
                        sprintf('%1.*e',sigmaz-1,z), ...
                        arrayrep(sigmay,isnan(sigmay),1), ...
                        y,'Uni',false), ...
                '((\d|\.)0*e[\+-][\d]{1,3})|(NaN|[-]?Inf)$')))-1, ...
                abs(x),oom(x)-oom(eps(x)));
pre = @(x) ... % Precision of x
    oom(x)-sig(x)+1;

% Checks inputs/outputs number:
assert(nargin<=4, 'CVARGAS:cbunits:tooManyInputs',...
...
for icb = 1:Ncbh
    ...
    cbh    = double(CBH(icb));
    units  = UNITS{icb}; if strcmp(units,'1'), units = ''; end
    prefix = '';
    sufix  = '';
    ...
    switch PREFIX{1,1}
        case ''
            % Adds the scientific notation to the unit like '/1e...':
            if strcmp(get(cbh,[XYstr 'Scale']),'linear')
                ind = find(ticks~=0,1);
                ord = oom(ticks(ind)/str2double(labels(ind,:)));
                if ord~=0 % Fixed bug 4.0 version
                    sufix = sprintf('/1e%d',ord);
                    if isempty(units)
                        units = '1';
                    end
                end
        ...
        case 'prefix'
            % Adds the scientific notation as IS prefix like 'M...':
            if strcmp(get(cbh,[XYstr 'Scale']),'linear')
                ind = find(ticks~=0,1);
                ord = oom(ticks(ind)/str2double(labels(ind,:)));
                if ord~=0
                    ...
                    labels = num2str(labels,sprintf('%%.%df',ndec));
                end
            end
        otherwise
    ...
    if (strcmp(XYstr,'Y') && ((abs(ticks(1))>abs(ticks(Nt))) && ...
            (length(fliplr(deblank(fliplr(labels( 1,:))))) > ...
            length(fliplr(deblank(fliplr(labels(Nt,:)))))))) || ...
            (strcmp(XYstr,'X') && strcmp(get(cbh,[XYstr 'Dir']),'reverse'))
        loc = 1;
    end
    labels = strjust(labels,'right');
    labels = [labels repmat(' ',Nt,Ne+Np+Nu+Ns)];
