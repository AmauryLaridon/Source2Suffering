...
% then compute whether differences between var1 and var2 are
% statistically significant using the signrank test function
% SREX regions are defined in Seneviratne et al. (2012: fig 3.1)
% --------------------------------------------------------------------


function [out] = mf_srex_statsign(lat, lon, var1, var2, area, island, user_mask, varargin)
...
% input to the function two input variables (2D or 3D) and their lat/lon  
...
for i=1:length(varargin)
    ...
    % check if srex masking needs to be done
    if strcmp(label, 'world')
        
        mask = ones(size(island));
    ...
    % get mean over time and take SQRT to get spatial RMSE
    var1_srex = sqrt(nanmean(var1, 3));
    var2_srex = sqrt(nanmean(var2, 3));

    
    % perform paired, two-sided Wilcoxon signed rank test at alpha % significance level (land pixels only)
    if   all(isnan(var1_srex(:))) || all(isnan(var2_srex(:)))   % no data to perform test on
        statsign_h = NaN;
    else
        [~, statsign_h] = signrank( var1_srex(mask) , var2_srex(mask) , 'alpha', alpha);
    end

    
    % retain value only if data was available for more than X% of all land pixels in the subdomain 
    npixels   = length(find(       var1_srex(mask)));
    nobs      = length(find(~isnan(var1_srex(mask))));
