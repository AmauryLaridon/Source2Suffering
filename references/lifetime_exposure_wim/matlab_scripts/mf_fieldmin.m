...
function [var_ap, varargout] = mf_fieldmin(var, varargin)
...
for i=1:length(varargin)
    ...
    if    length(size(var)) == 2
        ...
        % get field mean
        var_ap = nanmin(nanmin(var));
        var_mp = nanmin(nanmin(var(mask)));

    elseif length(size(var)) == 3

        % get field mean for all pixel
        var_ap = nanmin(nanmin(var,1),2);
        ...
        for j = 1:length(var(1,1,:))
            vari        = var(:,:,j);
            var_mp(j,1) = nanmin(nanmin(vari(mask),1),2);
        end
