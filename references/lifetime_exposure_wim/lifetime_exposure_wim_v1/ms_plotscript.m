...
flags.plot_4Nico_c  = 0;       % 0: do not plot heatwave exposure conceptual idea under OS and noOS scenarios
...
flags.plot_4Romania       = 1; % 0: do not plot bar chart for first expert report in Romania case
...
% save figure
export_fig('figures/for_nico/for_nico_conceptual.png', '-transparent');
...
for i=1:nruns
    ...
    % plot data
    if nanmax(RCP2GMT_diff_15  ) <= RCP2GMT_maxdiff_threshold
        plot(years_SR15, isimip(i).years(ind_RCP2GMT_15) , 'color', 'r', 'linewidth', 1); hold on;
    end
    if nanmax(RCP2GMT_diff_20  ) <= RCP2GMT_maxdiff_threshold
        plot(years_SR15, isimip(i).years(ind_RCP2GMT_20) , 'color', 'g', 'linewidth', 1); hold on;
    end
    if nanmax(RCP2GMT_diff_NDC ) <= RCP2GMT_maxdiff_threshold
