...
[unwpp_country_data] = xlsread('WPP2019_MORT_F16_1_LIFE_EXPECTANCY_BY_AGE_BOTH_SEXES',1);
[unwpp_region_data ] = xlsread('WPP2019_MORT_F16_1_LIFE_EXPECTANCY_BY_AGE_BOTH_SEXES',2);
unwpp_years          = (1952:5:2017)' - 5;                                               % assume block is 5 instead of reported 6 years to avoid overlap and take middle of that 5-year block (so 1952 for period 1950-1955). Substract 5 to get birth year of 5-year old (a 5-year old in 1952 was born in 1947 and we need the latter). hard coded from 'WPP2019_MORT_F16_1_LIFE_EXPECTANCY_BY_AGE_BOTH_SEXES_orig.xls'


% load Wittgenstein Center population size per age cohort (source: http://dataexplorer.wittgensteincentre.org/wcde-v2/)
[wcde_country_data] = xlsread('wcde_data',1);
[wcde_region_data ] = xlsread('wcde_data',2);
wcde_years          = (1950:5:2100)';                                                     % hard coded from 'wcde_data_orig.xls'
wcde_ages           = (2:5:102)';                                                         % hard coded from 'wcde_data_orig.xls' not that we assume +100 to be equal to 100-104
...
if max(years_SR15) < year_end 
    GMT_SR15   = cat(2, GMT_SR15, repmat( mean(GMT_SR15(:, end-9:end), 2), 1, year_end - max(years_SR15))); % repeat average of last 10 years (i.e. end-9 to end ==> 2090:2099)
    years_SR15 = [years_SR15'; (max(years_SR15)+1:1:year_end)'];
end
...
if max(years_UVIC) < year_end 
    GMT_UVIC   = cat(1, GMT_UVIC, repmat( mean(GMT_UVIC(end-9:end, :), 1), year_end - max(years_UVIC), 1)); % repeat average of last 10 years (i.e. end-9 to end ==> 2090:2099)
    years_UVIC = [years_UVIC; (max(years_UVIC)+1:1:year_end)'];
end
...
if max(years_pop) < year_end
    population = cat(3, population, repmat( mean(population(:,:, end-9:end), 3), 1, 1, year_end - max(years_pop))); % repeat average of last 10 years (i.e. end-9 to end ==> 2090:2099)
    years_pop  = [years_pop; (max(years_pop)+1:1:year_end)'];                                                          % order matters, compute this only after AFA and GMT!!!
end
...
if flags.runs == 1
    ...
    % initialise counter
    counter = 1;
%     counter = 250;  % for Heatwaves and TCs   - make sure to have the isimip struct loaded!


    % loop over extremes
    for i=1:length(extremes)    
%     for i=5:6   % for Heatwaves and TCs   
...
%             for k=1:1 % for testing
                ...
                GMT = GMT - mean( [GMT_pic(find(GMT_pic(:,1) == year_start_GMT_ref):find(GMT_pic(:,1) == GMT_his(1,1))-1,2); GMT_his(1:find(GMT_his(:,1) == year_end_GMT_ref),2)] );

                
                % if needed, repeat last year until entire period of interest is covered
                if max(years) < year_end
                    AFA   = cat(3, AFA, repmat( mean(AFA(:,:, end-9:end), 3), 1, 1, year_end - max(years))); % repeat average of last 10 years (i.e. end-9 to end ==> 2090:2099)
                    GMT   = [GMT; repmat( mean(GMT(end-9:end)), year_end - max(years), 1)];                      % repeat average of last 10 years (i.e. end-9 to end ==> 2090:2099)
                    years = [years; (max(years)+1:1:year_end)'];                                                 % order matters, compute this only after AFA and GMT!!!
                end
