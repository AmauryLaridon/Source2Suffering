...
function [exposure_mmm, exposure_mms, EMF_mmm, EMF_q25, EMF_q75, exposure_q25, exposure_q75, EMF_mmm_harmmean, EMF_mmm_geomexp] = mf_exposure_mmm(extremes, isimip_extreme, ind_rcp, ages, age_ref, exposure, exposure_pic_mean, RCP2GMT_maxdiff, RCP2GMT_maxdiff_threshold)
...
% % % test RCP2GMT_maxdiff_threshold - no longer works
% % figure;
% % for i=1:nruns
...
for i=1:nextremes    
    ...
    ind_extremes(:,i) = ismember(isimip_extreme, extremes{i})'; %#ok<*AGROW>

    
    % compute multi-model mean (mmm) per extreme impact category
    exposure_mmm(i,:,:) = mean(exposure(ind_rcp & ind_extremes(:,i), :, :), 1);


    % compute multi-model standard deviation (mms) per extreme impact category
    if length(find(ind_rcp & ind_extremes(:,i)==1)) > 1    
        exposure_mms(i,:,:) = std(exposure(ind_rcp & ind_extremes(:,i), :, :), 0, 1);
    else
        exposure_mms(i,:,:) = zeros(1,nregions,nbirthyears);
    end
...
% exposure_mms(nextremes + 1,:,:) = sum(exposure_mmm(1:nextremes,:,:), 1); % original approach: sum extremes
...
% exposure_mms(nextremes + 1,:,:) = sqrt(sum( exposure_mms(1:nextremes,:,:).^2 , 1));
...
exposure_mms(nextremes + 1,:,:) = sqrt(sum( (1./nextremes).^2 .* exposure_mms(1:nextremes,:,:).^2 , 1));


% compute multi-model IQR (q25 and q75) per extreme impact category
% exposure_q25(nextremes + 1,:,:) = sum(exposure_q25(1:nextremes,:,:), 1); % original approach: sum extremes
% exposure_q75(nextremes + 1,:,:) = sum(exposure_q75(1:nextremes,:,:), 1); % original approach: sum extremes
