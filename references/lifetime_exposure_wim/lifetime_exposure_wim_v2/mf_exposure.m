...
for j=1:ncountries 
    ...
    for k=1:nbirthyears
        ...
        ind_death = find(isimip_years == (birth_years(k) + floor(countries.life_expectancy_5{j}(k))), 1, 'first');
        
        
        % integrate exposure over full years lived
        exposure_perlife(1,j,k) = sum(exposure_peryear(1,j, ind_birth:ind_death));

        
        % add exposure during last (partial) year
        exposure_perlife(1,j,k) = exposure_perlife(1,j,k) + exposure_peryear(1,j, ind_death + 1) .* (countries.life_expectancy_5{j}(k) - floor(countries.life_expectancy_5{j}(k)));
