...
exposure_peryear_pic_map = mean(isimip_AFA_pic, 3);
...
for j=1:ncountries 
    ...
    exposure_perlife_pic_map = exposure_peryear_pic_map .* countries.life_expectancy_5{j}(1);

    
    % get indices of birth and deadth years
    ind_birth = find(isimip_years ==  birth_years(ind_year)                                                 , 1, 'first');
    ind_death = find(isimip_years == (birth_years(ind_year) + floor(countries.life_expectancy_5{j}(ind_year))), 1, 'first');


    % integrate exposure over full years lived - spatial map per country
    exposure_perlife_map = sum(isimip_AFA(:,:, ind_birth:ind_death), 3);


    % add exposure during last (partial) year - spatial map per country
    exposure_perlife_map = exposure_perlife_map + isimip_AFA(:,:, ind_death + 1) .* (countries.life_expectancy_5{j}(ind_year) - floor(countries.life_expectancy_5{j}(ind_year)));
