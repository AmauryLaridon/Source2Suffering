...
%     for i=250:nruns % heatwaves & TCs
        ...
        RCP2GMT_maxdiff_15(i,1)      = max(RCP2GMT_diff_15     );
        RCP2GMT_maxdiff_20(i,1)      = max(RCP2GMT_diff_20     );
        RCP2GMT_maxdiff_NDC(i,1)     = max(RCP2GMT_diff_NDC    );
        RCP2GMT_maxdiff_OS(i,1)      = max(RCP2GMT_diff_OS     );
        RCP2GMT_maxdiff_noOS(i,1)    = max(RCP2GMT_diff_noOS   );
        RCP2GMT_maxdiff_R26eval(i,1) = max(RCP2GMT_diff_R26eval);
        ...
        if flags.embers == 1
            for l=1:nGMTsteps
                ...
                % Get maximum T difference between RCP and GMT trajectories (to remove rows later)
                RCP2GMT_maxdiff_BE(i,l) = max(RCP2GMT_diff_BE(l,:));
