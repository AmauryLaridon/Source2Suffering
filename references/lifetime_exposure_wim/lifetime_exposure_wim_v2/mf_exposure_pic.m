...
for i=1:nruns_pic
    ...
    for j=1:ncountries 
        mask_life_expectancy(1 : round(countries.life_expectancy_5{j}(ind_age)), j) = 1;
    end
    ...
    [exposure_perlife_perrun_pic(:,:,i), ~] = bootstrp(nbootstraps, @(x)[ sum(x .* mask_life_expectancy, 1) ], cell2mat(exposure_peryear_pic(i,:)));
...
exposure_perlife_perrun_pic_mean = permute(mean(exposure_perlife_perrun_pic, 1), [3 2 1]);


% % % % get std of the bootstrapped distribution - per pic run
% % % exposure_perlife_perrun_pic_std = permute(std(exposure_perlife_perrun_pic), [3 2 1]);
...
for i=1:nextremes    
    ...
    for j=1:ncountries
        ...
        exposure_perlife_pic_mean(i,j) = mean(exposure_perlife_pic{i,j});
        exposure_perlife_pic_std(i,j)  = std( exposure_perlife_pic{i,j});
...
for j=1:ncountries
    ...
    for k=1:ncombinations
        ...
        % exposure_perlife_pic_allextremes(:,k) = sum(exposure_perlife_perrun_pic(:,j,random_run), 3); % original approach: sum extremes
        exposure_perlife_pic_allextremes(:,k) = geomean(exposure_perlife_perrun_pic(:,j,random_run), 3, 'omitnan'); % new approach: geometric mean, only used for getting the 'percause' analysis !! yields zero whenever one impact category is zero, e.g. TCs in MENA countries !!!
        % exposure_perlife_pic_allextremes(:,k) = mean(exposure_perlife_perrun_pic(:,j,random_run), 3, 'omitnan'); % final approach: arithmetic mean, only used for getting the 'percause' analysis
    ...
    % get mean and std for Burning embers
    exposure_perlife_pic_mean(nextremes+1,j)  = mean(exposure_perlife_pic{nextremes+1,j});
    exposure_perlife_pic_std(nextremes+1,j)   = std( exposure_perlife_pic{nextremes+1,j});
