...
elseif flags_text == 2   % letter & extreme - main figure 2
    ...
    % add panel letter (except for coldwaves)
    if mean(EMF_region_plot_BE, 'all') > 0.8
        % text(ages(1),GMT_BE(end,end) + 0.03, alphabet(ind_extreme),'ver','bottom','hor','left','Fontsize', 16, 'Fontweight', 'bold', 'color', axcolor)
        text(ages(1),GMT_BE(end,end) + 0.03, panelletters_regions(ind_extreme),'ver','bottom','hor','left','Fontsize', 16, 'Fontweight', 'bold', 'color', axcolor)
    else
        disp('assuming coldwaves')
        text(ages(1),GMT_BE(end,end) + 0.03, 'a','ver','bottom','hor','left','Fontsize', 16, 'Fontweight', 'bold', 'color', axcolor)
    end
...
if ind_extreme == 5 % heatwaves
    BE_ticks = [1/10 1/5 1/2 1 10 20 30];
else
...
elseif mean(EMF_region_plot_BE, 'all') < 0.8
    disp('assuming coldwaves, not adding pictogram')
    pictogram = imadjust(imcomplement(imread(['pictogram_' extremes{ind_extreme} '.png'])),[0 0 0; 0.01 0.01 0.1],[]);
else
