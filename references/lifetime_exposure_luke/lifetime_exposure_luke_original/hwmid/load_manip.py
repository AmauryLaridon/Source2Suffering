...
#%% ----------------------------------------------------------------
def groupingfunc(da):
    return xr.where(da.notnull(),len(da[~np.isnan(da)]),np.nan)

#%% ----------------------------------------------------------------
def nan_splits(da):
    return da.groupby(da.isnull().cumsum(dim='time')).map(groupingfunc)

#%% ----------------------------------------------------------------
def mgt_sum_max(da):
    da = da.groupby(da.isnull().cumsum(dim='time')).sum(dim='time')
...
def hot_period(
    ...
    # empty dataset for magnitudes            
    ds_mgt = xr.Dataset(
        coords={
            'lat': ('lat',ds_qntls.lat.data),
            'lon': ('lon',ds_qntls.lon.data),
        },
    )   
    ...
    for i in ids:
        ...
        da_90_cmp = xr.concat(
            [
                ds_qntls['90_{}'.format(i)].assign_coords({'dayofyear':xr.cftime_range(
                        str(y),
        ...
        # step 1, get hot days
        ds_daily['time'] = da_90_cmp['time'] # make calendars the same
        da_hd = xr.where( # days where temp exceeds 90th qntl of PIC 31 day window converted to 1
            ds_daily['tasmax_{}'.format(i)]>da_90_cmp,
            1,
            np.nan,
        ).chunk({'lat':lat_chunk,'lon':lon_chunk,'time':time_chunk}) # make sure time here is same as ds daily
        
        # step 2, get hot periods        
        da_hd = xr.map_blocks( # group data between nans (1s), replace each group's value by length of group
            nan_splits,
            da_hd,
        )
        da_hp = xr.where( # keep only groups with at least 3 days (hot periods)
            da_hd>=3,
            1,
            np.nan
        ) 
        da_hp = ds_daily['tasmax_{}'.format(i)].where(da_hp==1) # keep the daily temp data where/when hot periods occured
        
        # step 3, calculate magnitudes for hot periods, insert into dataset
        ds_mgt['i'] = xr.map_blocks(
            magnitude,
            da_hp,
            args=[ds_qntls['25_{}'.format(i)],ds_qntls['75_{}'.format(i)]],
            template=ds_ann['tasmax_{}'.format(i)].chunk({'lat':lat_chunk,'lon':lon_chunk,'time':time_chunk}),
        ).load()
