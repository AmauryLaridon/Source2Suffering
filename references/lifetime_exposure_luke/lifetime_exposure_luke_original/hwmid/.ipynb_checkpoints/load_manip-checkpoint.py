...
#     ds = ds.chunk({'time':-1})
#     ds = ds.chunk({'time':365}) # fix to chunk annually after the config.set chunking ; 265*36*72 elements is just <1,000,000
...
def collect_arrays(
    ...
    for i in ids:
        ...
        for f in files:
            ...
            if int(yi) <= 2091: # limiting data to 2099
                ...
                daily_data[i].append(open_daily_array(f,yi,lat_chunk,lon_chunk,time_chunk))

        daily_data[i] = xr.concat(daily_data[i],dim='time')#.chunk({'time':time_chunk}) # chunk again after concat
        
    ds_daily = xr.merge([daily_data[i].rename({'tasmax':'tasmax_{}'.format(i)}) for i in ids])
    ...
    for i in ids:
        ...
        ann_data[i] = open_annual_array(file,yi)
        
    ds_ann = xr.merge([ann_data[i].drop('time_bnds') for i in ids])
...
#         da_max = xr.map_blocks(
#             year_max,
#             ds[da],
#             template=xr.ones_like(ds[da].groupby('time.year').max('time').chunk({'year':-1}))
#         ).load()
#         print("--- {} minutes for computing da_max with {}-degree sized lat chunks---".format(
...
#         os.chdir(cdir)
#         with open('./data/pickles/max_{}.pkl'.format(da), 'wb') as f:
#             pk.dump(da_max_test,f)    
        ...
        print("--- {} minutes for computing 90th percentiles with {}-degree sized lat chunks for {}---".format(
            np.floor((time.time() - start_time) / 60),
            lat_chunk,
            i
        )
